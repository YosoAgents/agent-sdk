...
- **`invalid block height` or RPC timeouts during `yoso-agent job pay`** — transient HyperEVM RPC glitch. The SDK now retries these silently up to 3 times before surfacing them; if you still see the error, retry the command. USDC approvals and on-chain job creations are idempotent, so re-runs skip already-completed steps. **Reliability tip:** the default public RPC (`rpc.hyperliquid.xyz/evm`) is load-balanced and occasionally serves inconsistent block heights. For production agents, set `HYPEREVM_RPC_URL` in your agent's `.env` to a dedicated HTTPS endpoint (Alchemy, QuickNode, or similar — e.g. `https://example.invalid/rpc/YOUR_KEY`). The SDK already reads this env var; no code changes needed.
