  ...
  if (!deployInfo?.railwayConfig) {
    output.fatal(
      `No Railway project for agent "${agent.name}".\n` +
        "  Run `yoso-agent serve deploy railway setup` first."
    );
  }
...
export async function setup(): Promise<void> {
  ...
  if (existing) {
    ...
    output.log("  Run `yoso-agent serve deploy railway` to deploy.\n");
    return;
  }
  ...
  if (railwayConfig) {
    ...
    railway.initProject(`yoso-${agent.name}`);
  ...
  output.output({ provider: "railway", agent: agent.name, status: "setup_complete" }, () => {
    ...
    output.field("Railway Project", `yoso-${agent.name}`);
    output.log("\n  Next steps:");
    output.log("    1. Create offerings:   yoso-agent sell init <name>");
    output.log("    2. Register offering:  yoso-agent sell create <name>");
    output.log("    3. Deploy:             yoso-agent serve deploy railway\n");
  });
}
...
export async function deploy(): Promise<void> {
  ...
  if (offerings.length === 0) {
    output.fatal(
      "No offerings found. Create at least one offering first:\n\n" +
        "  yoso-agent sell init <name>\n" +
        "  yoso-agent sell create <name>"
    );
  }
  ...
  if (!railway.hasLinkedService()) {
    const projectName = `yoso-${agent.name}`;
    try {
  ...
  const config = readConfig();
  const apiKey = config.YOSO_AGENT_API_KEY;
  if (apiKey) {
    try {
      railway.setVariable("YOSO_AGENT_API_KEY", apiKey);
      output.success("Set YOSO_AGENT_API_KEY on Railway");
    } catch {
      output.warn(
        "Could not set YOSO_AGENT_API_KEY. Set it manually:\n" +
          "  yoso-agent serve deploy railway env set YOSO_AGENT_API_KEY=<your-api-key>"
      );
    }
  }
  ...
  output.output({ provider: "railway", status: "deployed", agent: agent.name, offerings }, () => {
    ...
    output.log("");
    output.log("  Check status:  yoso-agent serve deploy railway status");
    output.log("  View logs:     yoso-agent serve deploy railway logs --follow");
    output.log("  Tear down:     yoso-agent serve deploy railway teardown\n");
  });
}
...
export async function teardown(): Promise<void> {
  ...
  output.output({ provider: "railway", agent: agent.name, status: "torn_down" }, () => {
    ...
    output.log("    yoso-agent serve deploy railway\n");
  });
}
...
export async function envSet(keyValue: string): Promise<void> {
  ...
  if (eqIdx === -1) {
    output.fatal("Invalid format. Use: yoso-agent serve deploy railway env set KEY=value");
  }
...
export async function envDelete(key: string): Promise<void> {
  ...
  if (!key) {
    output.fatal("Usage: yoso-agent serve deploy railway env delete <KEY>");
  }
