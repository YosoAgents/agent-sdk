...
import { ensureBountyPollCron, removeBountyPollCronIfUnused } from "../lib/bountyPollCron.js";
...
export async function createInteractive(query?: string, sourceChannel?: string): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      if (cronRegistered) {
        output.log("  Bounty poll cron job registered for `yoso-agent bounty poll`.\n");
      }
    } catch {
    ...
    output.output(
      {
        ...
        ...(!cronRegistered
          ? {
              cronAction: "required",
              cronCommand: `yoso-agent cron add --id "yoso-bounty-poll" --schedule "*/10 * * * *" --command "cd \\"${ROOT}\\" && npx yoso-agent bounty poll --json"`,
            }
          : {}),
      },
      (data) => {
        ...
        if (!cronRegistered) {
          ...
          output.log(
            `    yoso-agent cron add --id "yoso-bounty-poll" --schedule "*/10 * * * *" --command "cd \\"${ROOT}\\" && npx yoso-agent bounty poll --json"`
          );
        }
        output.log("");
      }
...
export async function list(): Promise<void> {
  ...
  output.output({ bounties }, (data) => {
    ...
    for (const b of data.bounties) {
      ...
      if (b.jobId) output.field("Job ID", b.jobId);
      output.log("");
    }
  });
}
...
export async function poll(): Promise<void> {
  ...
  for (const b of bounties) {
    result.checked += 1;
    try {
      // --- Claimed bounties: track job status ---
      if (b.status === "claimed" && !b.jobId) {
        const remote = await getMatchStatus(b.bountyId);
        const remoteJobId = String(remote.job_id ?? "");
        if (remoteJobId) {
          saveActiveBounty({ ...b, jobId: remoteJobId });
          b.jobId = remoteJobId;
        } else {
          result.errors.push({
            bountyId: b.bountyId,
            error: "Bounty is claimed but missing jobId — cannot track job status",
          });
          continue;
        }
      }
      if (b.status === "claimed" && b.jobId) {
        ...
        try {
          const jobRes = await client.get(`/agents/jobs/${b.jobId}`);
          ...
          result.errors.push({
            bountyId: b.bountyId,
            error: `Failed to fetch job ${b.jobId} status`,
          });
          continue;
        }
      ...
      if (status === "claimed") {
        const remoteJobId = String(remote.job_id ?? "");
        if (remoteJobId) {
          saveActiveBounty({ ...b, status: "claimed", jobId: remoteJobId });
          result.claimedJobs.push({
            bountyId: b.bountyId,
            jobId: remoteJobId,
            title: b.title,
          ...
          result.errors.push({
            bountyId: b.bountyId,
            error: "Bounty is claimed but remote response missing job_id",
          });
        }
        continue;
      }
  ...
  output.output(result, (r) => {
    ...
    if (r.pendingMatch.length > 0) {
      ...
      for (const p of r.pendingMatch) {
        ...
        output.log(`      -> run: yoso-agent bounty select ${p.bountyId}`);
      }
    }
    if (r.rejectedByProvider.length > 0) {
      ...
      for (const rj of r.rejectedByProvider) {
        ...
        if (candidateCount > 0) {
          ...
          output.log(`      -> run: yoso-agent bounty select ${rj.bountyId}`);
        } else {
    ...
    if (r.claimedJobs.length > 0) {
      ...
      for (const j of r.claimedJobs) {
        output.log(
          `    - Bounty ${j.bountyId}: "${j.title}" — Job ${j.jobId} phase: ${j.jobPhase}`
        );
      }
    }
...
export async function status(bountyId: string, flags?: { sync?: boolean }): Promise<void> {
  if (!bountyId) output.fatal("Usage: yoso-agent bounty status <bountyId> [--sync]");
  ...
  const jobId = String(remote?.job_id ?? bounty.jobId ?? "");
  const matchedAgent = String(remote?.matched_agent ?? "");
  const candidates =
    status === "pending_match" && remote?.matched_agent_profile
      ? [remote.matched_agent_profile]
      : [];
  ...
  output.output(
    ...
    (data) => {
      ...
      if (data.jobId) output.field("Job ID", data.jobId);
...
export async function select(bountyId: string): Promise<void> {
  if (!bountyId) output.fatal("Usage: yoso-agent bounty select <bountyId>");
  ...
  try {
    ...
    const confirm = (await question(rl, "\n  Continue and create job for this candidate? (Y/n): "))
      .trim()
      .toLowerCase();
    ...
    const jobId = String(job.data?.data?.jobId ?? job.data?.jobId ?? "");
    if (!jobId) output.fatal("Failed to create job for selected candidate.");
    ...
    output.output(
      ...
      (data) => {
        ...
        output.field("Status", data.status);
        output.log(`\n  Use \`yoso-agent job status <jobId>\` to monitor the job.`);
        output.log(
          `  Then run \`yoso-agent bounty status ${data.bountyId} --sync\` to sync/update bounty status.\n`
        );
      }
    );
  } finally {
...
export async function update(bountyId: string, flags: BountyUpdateFlags): Promise<void> {
  if (!bountyId) output.fatal("Usage: yoso-agent bounty update <bountyId> [flags]");
...
export async function cancel(bountyId: string): Promise<void> {
  if (!bountyId) output.fatal("Usage: yoso-agent bounty cancel <bountyId>");
...
export async function cleanup(bountyId: string): Promise<void> {
  if (!bountyId) output.fatal("Usage: yoso-agent bounty cleanup <bountyId>");
  const active = getActiveBounty(bountyId);
  if (!active) {
