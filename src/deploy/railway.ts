...
export function checkCli(): { installed: boolean; version?: string } {
  try {
    const version = execFileSync("railway", ["--version"], {
      ...EXEC_OPTS,
...
export function login(): void {
  try {
    execFileSync("railway", ["login", "--browserless"], {
      ...EXEC_OPTS,
      stdio: "inherit",
    });
  } catch {
    // Fall back to browser-based login if --browserless fails
    execFileSync("railway", ["login"], { ...EXEC_OPTS, stdio: "inherit" });
  }
}

export function initProject(name?: string): void {
  const args = name ? ["init", "--name", name] : ["init"];
  execFileSync("railway", args, { ...EXEC_OPTS, stdio: "inherit" });
}
...
export function linkService(name: string): void {
  execFileSync("railway", ["service", "link", name], {
    ...EXEC_OPTS,
    stdio: ["pipe", "pipe", "pipe"],
  });
}
...
export function hasLinkedService(): boolean {
  try {
    const status = execFileSync("railway", ["status"], {
      ...EXEC_OPTS,
      stdio: ["pipe", "pipe", "pipe"],
    }).trim();
...
export function listVariables(): string {
  return execFileSync("railway", ["variables"], {
    ...EXEC_OPTS,
    stdio: ["pipe", "pipe", "pipe"],
  }).trim();
}
...
export function up(): void {
  execFileSync("railway", ["up", "--detach"], { ...EXEC_OPTS, stdio: "inherit" });
}

export function getStatus(): string {
  return execFileSync("railway", ["status"], {
    ...EXEC_OPTS,
    stdio: ["pipe", "pipe", "pipe"],
  }).trim();
}
...
export function streamLogs(follow: boolean, filter: LogFilter = {}): void {
  ...
  if (follow) {
    ...
    if (active) {
      const raw = execFileSync("railway", ["logs", "--lines", "50"], {
        ...EXEC_OPTS,
        stdio: ["pipe", "pipe", "pipe"],
      });
      ...
      execFileSync("railway", ["logs", "--lines", "50"], { ...EXEC_OPTS, stdio: "inherit" });
    }
  }
}

export function down(): void {
  execFileSync("railway", ["down", "--yes"], { ...EXEC_OPTS, stdio: "inherit" });
}
