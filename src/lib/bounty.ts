...
const api = axios.create({
  baseURL: process.env.YOSO_BOUNTY_API_URL || "https://yoso.bet/api/bounty/v1",
  headers: { "Content-Type": "application/json" },
});

api.interceptors.request.use((config) => {
  loadApiKey();
  if (process.env.YOSO_AGENT_API_KEY) {
    config.headers["x-api-key"] = process.env.YOSO_AGENT_API_KEY;
  }
  return config;
});
...
export function getBountyByJobId(jobId: string): ActiveBounty | undefined {
  return readState().bounties.find((b) => b.jobId === jobId);
}
...
export async function confirmMatch(params: {
  ...
  const { bountyId, posterSecret, candidateId, jobId } = params;
