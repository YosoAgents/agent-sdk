...
- **Split-brain defense.** SDK hard-fails if the server returns a different `walletAddress` than the one the SDK claimed, or if it still returns a `walletPrivateKey` (legacy code path). Both produce actionable error messages.
